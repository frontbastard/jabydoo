...
class ContentGenerationService:
    ...
    def generate_seo(self, page_title: str, page_content: str) -> dict:
        prompt = (
            ...
            f"for a website '{self.options.brand_name}' on the topic of '{self.options.activity}'.\n"
            f"Title: 50-60 characters.\nDescription: 140-160 characters.\n"
            f"Don't use placeholders like [site name].\n"
    ...
    def generate_content_for_page(self, request, page):
        ...
        if self.content_service.options.ai_image_model and not page.image:
            image_url = self.content_service.generate_image(
                f"Create an image for the article '{page.title}' related to '{self.content_service.options.activity}' "
                f"ensuring that no text or letters appear in the image."
            )
